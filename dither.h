...
// --- D�finitions globales et structures ---
#define NUM_THOMSON_COLORS 4096 // Nombre total de couleurs dans la palette Thomson
#define PALETTE_SIZE 16			// Nombre de couleurs souhait� dans la palette finale
...
typedef struct {
	int min_idx, max_idx;	  // Plage d'indices Thomson que cette bo�te couvre
	long sum_r, sum_g, sum_b; // Sommes des composantes RGB des couleurs Thomson dans cette bo�te
	uint32_t pixel_count;	  // Nombre total de pixels (r�els) associ�s aux couleurs Thomson dans cette bo�te
	double variance;		  // Variance des couleurs dans cette bo�te (utilis�e pour le crit�re de division)
} WuBox;

// --- D�claration de la palette Thomson globale et du tableau d'unicit� ---
